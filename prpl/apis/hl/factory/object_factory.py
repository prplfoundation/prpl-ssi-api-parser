...
class ObjectFactory:
    ...
    def __init__(self, procedures, fields, events, instances, response_codes, change_log):
        ...
        self.events = sorted(events, key=itemgetter('Layer', 'Object', 'Name'))
    ...
    def _get_events(self, object_name):
        ...
        while len(self.events) > 0:
            ...
            if object_name == e['Object']:
                # Pretty print sample.
                p_sample = json.dumps(json.loads(e['Sample']), indent=2, sort_keys=False)

                # Create new event.
                api_event = HLAPIEvent(e['Code'], e['Name'], e['Description'], p_sample)
    ...
    def _get_objects(self):
        ...
        while len(self.procedures) > 0:
            ...
            # Pretty print sample request body.
            m_sample_request = p['Request Body (Sample)']
            if m_sample_request != '-':
                try:
                    m_sample_request = json.dumps(json.loads(p['Request Body (Sample)']), indent=2, sort_keys=False)
                except Exception as e:
                    # Log sample in case of invalid sample.
                    self.logger.error('Objects - Invalid JSON detected on sample request body ({}:{}).'.format(
                        object_name,
                        p['Method']))
                    raise e

            # Pretty print sample response body.
            try:
                m_sample_response = json.dumps(json.loads(p['Response Body (Sample)']), indent=2, sort_keys=False)
            except Exception as e:
                # Log sample in case of invalid sample.
                self.logger.error('Objects - Invalid JSON detected on sample response body ({}:{}).'.format(
                    object_name,
                    p['Method']))
                raise e

            api_procedure = HLAPIProcedure(p['Method'], p['Description'], m_sample_request, m_sample_response)
    ...
    def _get_response_codes(self):
        ...
        while len(self.response_codes) > 0:
            ...
            try:
                # Pretty print sample.
                p_sample = json.dumps(json.loads(rc['Sample']), indent=2, sort_keys=False)
            except Exception as e:
                # Log sample in case of invalid sample.
                self.logger.error('Response Codes - Invalid JSON detected on "{}" sample.'.format(rc['Name']))
                raise e

            # Create new response code object instance and append.
            api_code = HLAPIResponseCode(rc['Name'], rc['Description'], p_sample)
    ...
    def _get_change_log(self):
        ...
        while len(self.change_log) > 0:
            ...
            self.logger.debug('ChangeLog - Added version "{} ({})" with {} changes.'.format(
                v.number,
                v.date,
                len(v.change_list)))
