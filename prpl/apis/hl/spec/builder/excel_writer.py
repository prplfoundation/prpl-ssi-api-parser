...
class ExcelWriter:
  ...
  def makeField(self, fields_sheet, api_object_layer, api_object_name, procedure_name, resource, field):
  ...
  def iterateThroughFields(self, wb, api_object_layer, api_object_name, procedure_name, resource, fields):
    ...
    for f in fields:
      ...
      self.makeField(fields_sheet, api_object_layer, api_object_name, procedure_name, resource, field)


  def iterateThroughObjects(self, wb):

    object_sheet = wb.get_sheet_by_name("Objects")
    fields_sheet = wb.get_sheet_by_name("Fields")
    ...
    for api_object in self.api.objects:
      ...
      for procedure in api_object.procedures:
        ...
        resource = re.sub(r'\{.+?\}\s?', "", api_object.resource.replace("."," "))

        # append new row with values
        object_sheet.append([api_object.layer, api_object.resource, procedure.name, procedure.sample_request, procedure.sample_response, sample, resource , procedure.description])
      ...
      for event in api_object.events:
        ...
        prefix = "{}_".format(api_object.name.upper().replace(".", "_"))
    ...
    for response_code in self.api.response_codes:
      response_code_sheet.append([response_code.raised_by, response_code.name, response_code.sample, response_code.description])

    # activate auto filter on all rows
    object_sheet.auto_filter.ref = "A1:C{}".format(object_sheet._current_row)

    fields_sheet.auto_filter.ref = "A1:C{}".format(fields_sheet._current_row)

    events_sheet.auto_filter.ref = "A1:F{}".format(events_sheet._current_row)
  ...
  def createSheets(self, wb):
    ...
    ws.append(["Layer", "Object", "Resource", "Code", "Prefix", "Event", "Name", "Parameters", "Sample", "Description"])
