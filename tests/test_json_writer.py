...
class TestJSONSchemaWriter(unittest2.TestCase):
    ...
    def test__check_user_components(self):
        ...
        for e in self.api_object.events:
            ...
            self.assertEqual(json.loads(e.sample), json.loads(
                self.user_accounts_json["components"]["schemas"][name]["events"][e.name]["content"]["application/json"][
                    "example"]))

            # check if the description is correct
            self.assertEqual(e.description,
                             self.user_accounts_json["components"]["schemas"][name]["events"][e.name]["description"])
